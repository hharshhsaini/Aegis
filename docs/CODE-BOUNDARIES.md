...
Package imports use `aegis`; `package.json` is the authoritative
...
`aegis/build/vite` is a separate Node-only export. `build/vite.js`
...
```js
import { createBrowserViteConfig } from 'aegis/build/vite';
...
helpers for compatibility. Read-only provider status lives in `server/standalone/key-setup.js`
...
The portable `aegis/sources/adsb-lol` export normalizes existing aircraft
...
`aegis/server/providers/places` exports the Google nearby-place and
...
`aegis/sources/places` exports portable response projections for Google
...
`aegis/server/providers/space` is a Node-only entry for the CelesTrak
...
`aegis/sources/space` owns only fixed upstream URL construction: the
...
`aegis/server/providers/terrain`, `/traffic`, `/firms` and `/gbfs`
...
`aegis/sources/terrain` exports existing point-key, retry and cache
reconstruction mechanics with injectable acquisition dependencies.
`aegis/sources/traffic` exports tile math and budget calculations.
`aegis/sources/gbfs` exports host/path acceptance and cache-header rules.
...
`aegis/sources/firms-csv` exports the existing CSV parser, header
...
`aegis/search` exports an explicit geocoding service and Google/Photon
...
`aegis/ui/panels` owns collapse-button binding, nearest-panel Escape
...
`aegis/ui/surfaces` exports `createSurfaceKeyboard` from
...
`aegis/ui/layout` exports synchronous `layoutLeftPanelRail` and
...
`aegis/ui/input` exports `bindApplicationShortcuts` and
...
`aegis/scenes` exports `SceneDirector`. Its `subscribe(listener)` supplies
...
`aegis/application/catalog` captures caller-supplied layer instances and
...
`aegis/sources/reference` constructs fresh earthquake and bundled cable
