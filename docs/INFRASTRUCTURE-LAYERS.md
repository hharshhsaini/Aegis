...
`aegis/infrastructure` exports `createInfrastructureLayers(services)`.
...
`aegis/infrastructure/geojson` exports the lower-level
`createLocalGeoJsonLayer(options, services)` and existing infrastructure overlay
helpers. `aegis/infrastructure/lod` exports the existing LOD policy helpers.
